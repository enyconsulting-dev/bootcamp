...
const countries = [
  "Nigeria", "United States", "Canada", "United Kingdom", "Australia", "Ghana", "South Africa", "Kenya", "Uganda", "Tanzania", "Rwanda", "Ethiopia", "Egypt", "Morocco", "France", "Germany", "Ireland", "Italy", "Netherlands", "Spain", "Switzerland", "United Arab Emirates", "Saudi Arabia", "India", "Pakistan", "Singapore", "Malaysia", "China", "Japan", "New Zealand", "Brazil", "Mexico", "Argentina", "Colombia", "Jamaica", "Trinidad and Tobago",
];
...
export default function WaitlistPage() {
  ...
  return <main className="waitlist-page">
    ...
    <section className="waitlist-section proof-section"><div className="waitlist-heading"><p className="waitlist-label dark">What people say</p><h2>They had the expertise too. Here&apos;s what happened when they used it.</h2></div><div className="testimonial-grid">{[1, 2, 3, 4, 5, 6].map((number) => <figure className="testimonial-image-card" key={number}><img src={`/Feedback_${number}.png`} alt={`Participant feedback from live masterclass, testimonial ${number}`} loading="lazy" /></figure>)}</div><div className="trust-bar">100,000+ professionals trained / 90+ countries / 95% land a client in 90 days</div></section>

    <section className="waitlist-section instructors-section"><div className="waitlist-heading"><p className="waitlist-label dark">Your instructors</p><h2>Built by someone who&apos;s done it. Led by someone who&apos;ll be there every week.</h2></div><div className="instructor-cards"><article><div className="instructor-avatar"><img src="/eno_headshot.jpeg" alt="Eno Eka" /></div><p className="waitlist-label dark">Founder</p><h3>Eno Eka</h3><p>CEO, ENY Consulting Inc. and founder of Consulting School and Business Analysis School. Eno has trained over 100,000 professionals across 90+ countries and led a $6 billion digital transformation in Canada.</p><p>She built this bootcamp for talented professionals who have not yet packaged what they know.</p></article><article><div className="instructor-avatar coach-avatar"><img src="/coach_jojo.jpeg" alt="Coach Jojo" /></div><p className="waitlist-label dark">Lead cohort coach</p><h3>Coach Jojo</h3><p>Josephine  is a Revenue Operations and AI Automation consultant with over four years of experience helping organizations and tech professionals scale their operations and market positioning. Bringing extensive expertise in sales, marketing, and AI-driven strategies, she has consulted on and optimized over 1,000 professional portfolios, profiles, and digital assets worldwide. Josephine partners with clients to build strong brand presence, streamline processes, and implement modern go-to-market frameworks.</p></article></div></section>

    <section className="waitlist-form-section" id="waitlist-form"><div className="waitlist-heading"><p className="waitlist-label">Save your spot</p><h2>Join the free waitlist - and get your bonus right now.</h2><p>It is free, takes 30 seconds, and your waitlist bonus lands in your inbox the moment you submit.</p></div><form onSubmit={submit} className="waitlist-form">{[["first_name", "First name"], ["last_name", "Last name"], ["email", "Email address"], ["country", "Country"], ["phone", "Phone (WhatsApp)"]].map(([name, label]) => <label key={name}>{label}<input required name={name} type={name === "email" ? "email" : "text"} list={name === "country" ? "country-list" : undefined} autoComplete={name === "country" ? "country-name" : undefined} value={form[name as keyof typeof form]} onChange={(event) => setForm({ ...form, [name]: event.target.value })} />{name === "country" && <datalist id="country-list">{countries.map((country) => <option key={country} value={country} />)}</datalist>}</label>)}{error && <p className="form-error" role="alert">{error}</p>}<button className="waitlist-cta" disabled={submitting}>{submitting ? "Saving your spot..." : "Join the waitlist - it is free"}<small>No payment now. Your information is private.</small></button></form><div className="form-trust">100,000+ trained / 90+ countries / 95% success rate / Founded by Eno Eka</div></section>
