...
export default function EnrollmentSalesPage() {
  ...
  return <main className="sales-page">
    ...
    <section className="sales-hero"><div className="sales-hero-inner"><p className="sales-eyebrow">Waitlist members only <i /> Early-bird access <i /> Enrollment opens September 15th</p><p className="sales-kicker">30-Day Consulting Offer Bootcamp</p><h1>You&apos;re on the list.<br /><em>Here&apos;s what you just unlocked.</em></h1><p className="sales-lede">When enrollment opens on September 15th, you&apos;ll have 48 hours to enroll at the early-bird price before the public knows enrollment is open. This page shows you exactly what you&apos;re enrolling in.</p><div className="sales-price-card"><span>{label} early-bird price</span><strong>{earlyPrice}</strong><del>{regularPrice}</del><small>Early-bird closes September 22nd</small><Countdown /></div><EnrollmentButton currency={currency} /><p className="sales-microcopy">Your local price is shown automatically. Secure checkout.</p></div><div className="trust-strip sales-trust"><span>100,000+ professionals trained</span><span>90+ countries</span><span>95% land a client in 90 days</span><span>Secure checkout</span></div></section>

    <section className="sales-section sales-video"><div className="sales-heading"><p className="sales-label">From Eno - watch this first</p><h2>You&apos;re on the waitlist. Before you read the rest, watch this.</h2><p>Eno recorded a short message specifically for waitlist members. Under three minutes. Worth every second.</p></div><div className="video-placeholder" role="img" aria-label="Welcome video placeholder"><div className="play-button">Play</div><p>Eno&apos;s welcome video</p><small>The video will be connected before launch.</small></div><p className="sales-caption">Watch before you read anything else. It changes how you see what you already have.</p></section>
    ...
    <section className="sales-section sales-proof"><div className="sales-heading"><p className="sales-label dark">What people say</p><h2>They had the expertise too. Here&apos;s what happened when they packaged it.</h2></div><div className="sales-testimonials">{["$12,000 client. Week 5.", "In 30 days I had an offer and two discovery calls booked.", "The live Saturday feedback changed everything.", "I had been giving my expertise away for years.", "The accountability pushed me to finish.", "My first real client call felt easy after the practice."].map((quote) => <blockquote key={quote}><span>Testimonial placeholder</span><p>{quote}</p><cite>Name, title, country</cite></blockquote>)}</div><div className="trust-bar">100,000+ professionals trained / 90+ countries / 95% land a client in 90 days</div><EnrollmentButton currency={currency} className="cta coral-dark" /></section>
